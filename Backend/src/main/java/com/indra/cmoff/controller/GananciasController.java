...
public class GananciasController {
	...
	public GananciasController(IGananciaService gananciaService) {
	...
	public Optional<GananciasDTO> one(@PathVariable Long codEmpleado) {
		return  gananciaService.findById(codEmpleado);
	}
	...
	public GananciasDTO findByCodigo(@PathVariable Long codigo) {
		return  gananciaService.findByCodEmpleado(codigo);
	}

	@PreAuthorize("hasRole('"+ ConstantesRolesMP.ROLE_ADM +"') "
			+ "or hasAuthority('"+ ConstantesRolesMP.ADMU + ConstantesRolesMP.PRM_L +"')")
	@PostMapping("/filternombres")
	public Page<GananciasDTO> filtrarPorNombre(@RequestBody PaginatedFilter<GananciasDTO> filtro) {
		return  gananciaService.filtroPagPorNombre(filtro.getPage(), filtro.getSizePerPage(), filtro.getFilter(),
	...
	@PostMapping("/buscar")
	public Page<GananciasDTO> buscar(@RequestBody PaginatedFilter<GananciasDTO> filtro) {
		return  gananciaService.filterPaginated(filtro.getPage(), filtro.getSizePerPage(), filtro.getFilter(),
				filtro.getColumn(), filtro.getOrder());
	...
	@PostMapping("/crear")
	public Object createNew(@Valid @RequestBody GananciasDTO newGanancias) {

		try {
			gananciaService.save(newGanancias);
